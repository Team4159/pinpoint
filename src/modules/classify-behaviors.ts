...
const autoClasses: BehaviorClass[] = [
  {
    test: robotEntry => robotEntry.autoScaleCubes > 0 && robotEntry.autoSwitchCubes > 0,
    name: robotEntry => `${robotEntry.autoSwitchCubes} Cube Scale & ${robotEntry.autoSwitchCubes} Cube Switch Auto`
  },
  {
    test: robotEntry => robotEntry.autoScaleCubes > 0 && robotEntry.autoSwitchCubes == 0,
  ...
  {
    test: robotEntry => robotEntry.autoSwitchCubes > 0 && robotEntry.autoScaleCubes == 0,
  ...
  for (let robotEntry of robotEntries) {
    for (let autoClass of autoClasses) {
      if (autoClass.test(robotEntry)) {
        const autoBehavior = typeof autoClass.name == 'function' ? autoClass.name(robotEntry) : autoClass.name;
